"""Gradio web front-end for ClipRadar.
...
Free hosting note: Hugging Face Spaces' CPU Basic tier (Gradio and Docker
SDKs) is no longer free for new accounts as of Aug 2026 — only the Static
and ZeroGPU SDKs are. See FREE_HOSTING.md for the current genuinely-free
options (Colab tunnel, Cloud Run, etc). This file works unchanged on all
of them.

Run locally:  python app.py
Run with a public tunnel (e.g. from Colab): set CLIPRADAR_SHARE=1
...
if __name__ == "__main__":
    # 0.0.0.0 + $PORT so this also works unmodified inside a container
    # (Render/Cloud Run/Fly.io all inject PORT or expect a fixed listen port).
    #
    # CLIPRADAR_SHARE=1 flips on Gradio's built-in public tunnel
    # (a temporary https://xxxx.gradio.live link, no hosting account needed
    # at all). That's the path for running this out of a Google Colab
    # notebook — see clipradar_colab.ipynb / FREE_HOSTING.md. Leave it off
    # when running inside a real container host, which already gives you a
    # stable public URL and doesn't need a tunnel.
    share = os.environ.get("CLIPRADAR_SHARE", "0") == "1"
    demo.queue().launch(
        server_name=os.environ.get("CLIPRADAR_HOST", "0.0.0.0"),
